...
require 'misc.DataLoaderCNN'
require 'misc.LanguageModelCNN'
require 'misc.LanguageModelCNN_Only'
require 'misc.LanguageModelCNN_DeepFusion'
...
cmd:option('-prelm_model', '', 'path to a model checkpoint of lm. Empty = don\'t')
...
-------------------------------------------------------------------------------
local loader = DataLoaderCNN{h5_file = opt.input_h5, json_file = opt.input_json}
...
local prelm_ck_point = torch.load(opt.prelm_model)
local prelm = prelm_ck_point.protos.lm
...
if string.len(opt.start_from) > 0 then
  ...
  protos.lm = nn.LanguageModelCNN(lmOpt)
...
local function lossFun()
  ...
  local dexpanded_feats, ddummy = unpack(protos.lm:backward({expanded_feats, data.labels, data.chars, loader.char_to_ix}, dlogprobs))
  ...
  if (iter % opt.save_checkpoint_every == 0 or iter == opt.max_iters) then
    ...
    if best_score == nil or current_score > best_score then
      ...
      if iter > 0 then -- dont save on very first iteration
        ...
        --save_protos.prelm = prelm  -- Just save the prelm as well
