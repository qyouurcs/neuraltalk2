...
local DataLoaderCNN = torch.class('DataLoaderCNN')

function DataLoaderCNN:__init(opt)
  
  -- load the json file which contains additional information about the dataset
  print('DataLoaderCNN loading json file: ', opt.json_file)
  ...
  -- open the hdf5 file
  print('DataLoaderCNN loading h5 file: ', opt.h5_file)
...
function DataLoaderCNN:resetIterator(split)
  self.iterators[split] = 1
end

function DataLoaderCNN:getVocabSize()
  return self.vocab_size
end

function DataLoaderCNN:getCharVocabSize()
  return self.char_vocab_size
end

function DataLoaderCNN:getMaxWordL()
  return self.max_word_l
end

function DataLoaderCNN:getCharVocab()
  return self.ix_to_char
end


function DataLoaderCNN:getVocab()
  return self.ix_to_word
end

function DataLoaderCNN:getSeqLength()
...
--]]
function DataLoaderCNN:getBatch(opt)
  ...
  local char_batch = torch.LongTensor(batch_size * seq_per_img, self.seq_length, self.max_word_l)
  ...
  for i=1,batch_size do
    ...
    if ncap < seq_per_img then
      ...
      char_seq = torch.LongTensor(seq_per_img, self.seq_length, self.max_word_l)
      for q=1, seq_per_img do
        local ixl = torch.random(ix1,ix2)
        seq[{ {q,q} }] = self.h5_file:read('/labels'):partial({ixl, ixl}, {1,self.seq_length})
        char_seq[{ {q,q},}] = self.h5_file:read('/chars'):partial({ixl, ixl}, {1,self.seq_length}, {1,self.max_word_l})
      end
    else
      ...
      char_seq = self.h5_file:read('/chars'):partial({ixl, ixl+seq_per_img-1}, {1,self.seq_length}, {1,self.max_word_l})
    end
    local il = (i-1)*seq_per_img+1
    label_batch[{ {il,il+seq_per_img-1} }] = seq
    char_batch[{ {il,il+seq_per_img-1},{} }] = char_seq
