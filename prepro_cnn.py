...
def encode_captions(imgs, params, wtoi):
  ...
  for i,img in enumerate(imgs):
    ...
    chars_i = np.zeros((n, max_length, max_word_l), dtype='uint32')
      ...
      for k,w in enumerate(s):
        if k < max_length:
          ...
          for kk, cc in enumerate(w): 
            ...
            if kk + 1 < max_word_l:
              chars_i[j,k,kk + 1] = char2idx[cc]
          last_idx = min(max_word_l-1, kk + 2)
          #pdb.set_trace()
          chars_i[j,k,last_idx] = char2idx[end]

    # note: word indices are 1-indexed, and captions are padded with zeros
    label_arrays.append(Li)
    label_chars_array.append(chars_i)
  ...
  C = np.concatenate(label_chars_array, axis = 0) # put all the chars together.
  assert L.shape[0] == M, 'lengths don\'t match? that\'s weird'
  assert np.all(label_length > 0), 'error: some caption had no words?'

  print 'encoded captions to array of size ', `L.shape`
  return L, label_start_ix, label_end_ix, label_length, C, idx2char, char2idx
...
def main(params):
  ...
  L, label_start_ix, label_end_ix, label_length, C, idx2char, char2idx = encode_captions(imgs, params, wtoi)
