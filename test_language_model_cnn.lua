...
local function forwardApiTestFactory(dtype)
  ...
  loadstring('opt.feature_maps = ' .. opt.feature_maps)() -- get feature map sizes
    ...
    opt.seq_length = 7
    opt.batch_size = 10
    local lm = nn.LanguageModelCNN(opt)
    ...
    local gradInput = lm:backward({imgs, seq, seq_chars, char_to_ix}, gradOutput)
...
local function gradCheckLM()
  ...
  local gradOutput = w
  local gradInput, dummy = unpack(lm:backward({imgs, seq, seq_chars, char_to_ix}, gradOutput))

  -- create a loss function wrapper
  local function f(x)
    local output = lm:forward{x, seq, seq_chars, char_to_ix}
    local loss = torch.sum(torch.cmul(output, w))
    return loss
  end
...
local function overfit()
  ...
  local function lossFun()
    ...
    local loss = crit:forward(output, seq)
    local gradOutput = crit:backward(output, seq)
    lm:backward({imgs, seq, seq_chars, char_to_ix}, gradOutput)
    return loss
  end
...
local function sample()
  ...
  local imgs = torch.randn(opt.batch_size, opt.input_encoding_size):type(dtype)
  local seq = lm:sample(imgs, char_to_ix, ix_to_word)
...
local function sample_beam()
  ...
  local seq_vanilla, logprobs_vanilla = lm:sample(imgs, char_to_ix, ix_to_word)
  local seq, logprobs = lm:sample(imgs, char_to_ix, ix_to_word, {beam_size = 1})
