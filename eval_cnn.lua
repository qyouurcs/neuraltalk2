...
cmd:option('-image_root', 'coco_all_images', 'In case the image paths have to be preprended with a root path to an image folder')
...
if string.len(opt.image_folder) == 0 then
  loader = DataLoaderCNN{h5_file = opt.input_h5, json_file = opt.input_json}
else
